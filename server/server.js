...
// AI Insights API endpoint - Generate AI insights for audit tool
app.post('/api/ai-insights/generate', async (req, res) => {
  try {
    ...
    console.log('[AI-INSIGHTS] Generating AI insights...');

    // Use Azure OpenAI configuration from environment
    const azureEndpoint = process.env.AZURE_OPENAI_ENDPOINT;
    const azureApiKey = process.env.AZURE_OPENAI_API_KEY;
    const azureDeployment = process.env.AZURE_OPENAI_DEPLOYMENT || 'gpt-4o-mini-3';
    const azureApiVersion = process.env.AZURE_OPENAI_API_VERSION || '2025-01-01-preview';

    if (!azureEndpoint || !azureApiKey) {
      console.error('[AI-INSIGHTS] Azure OpenAI not configured');
    ...
    if (!response.ok) {
      ...
      console.error('[AI-INSIGHTS] Azure OpenAI error:', response.status, errorText);
    ...
    if (!content) {
      return res.status(500).json({ error: 'No content generated by Azure OpenAI' });
    }
